...
TravelApp.controller('TravelController', function ($scope, $window, $http, $timeout, $sce, TravelService, TravelFactory,) {
  ...
  $scope.selectHotel = function (hotel) {
    // Find the full hotel details from HotelDetails array based on hotel name
    var fullHotelDetail = HotelDetails.find(h => h.title === hotel.HotelName);
    $scope.HotelDetailData = fullHotelDetail || hotel;
  };
  ...
  $scope.GetRestaurantsData = function (RIndex) {
    ...
    var coords = $scope.RestaurantDetail.detail.locationCode;
    if (coords && coords.lat && coords.lng) {
      // Clear the container
      RestaurantLocation.innerHTML = '';
      
      // Set proper dimensions
      RestaurantLocation.style.width = '100%';
      RestaurantLocation.style.height = '450px';
      
      // Use setTimeout to ensure DOM is ready before initializing map
      setTimeout(function() {
        // Initialize Leaflet map
        var map = L.map(RestaurantLocation).setView([coords.lat, coords.lng], 15);
