$('#collapseMap').on('shown.bs.collapse', function(e){
	...
	if (!Array.prototype.forEach)
		...
		markersData = {
			...
			var
			marker;
		mapObject = L.map('map').setView([39.8, -98.6], 4);
		...
		for (var key in markersData)
			markersData[key].forEach(function (item) {
				...
				marker = L.marker([item.location_latitude, item.location_longitude], {icon: customIcon}).addTo(mapObject);
				...
				marker.on('click', (function (m) {
					return function() {
						closeInfoBox();
						getInfoBox(item).open(mapObject, m);
						mapObject.setView([item.location_latitude, item.location_longitude], 12);
					};
				})(marker));
